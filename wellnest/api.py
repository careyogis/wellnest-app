...
def profile():
    ...
    for rating in caregiver_data.ratings:
        customer_data.append(frappe.get_doc("Customer", rating.rater))

    # If Caregiver is Solo and not from an Agency
    if caregiver[0].supplier:
        agency = frappe.get_doc("Supplier", caregiver[0].supplier)
        agency_contact = frappe.get_doc("Address", agency.supplier_primary_address)
        return {
            "caregiver_name": caregiver[0],
